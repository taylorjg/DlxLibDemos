...
public static class Pieces
{
  ...
  )
  {
    ...
    var lines = ParseLineDescriptions(lineDescriptions);
    ...
    foreach (var bit in bits)
    {
      ...
      switch (type)
      {
        case 'H':
          ...
          lineSegments.Add(new LineSegment(LineSegmentType.Horizontal, coords));
          break;
        case 'V':
          verticals.Add(coords);
          lineSegments.Add(new LineSegment(LineSegmentType.Vertical, coords));
          break;
        case 'I':
  ...
  private static int[][] ParseLineDescriptions(string lineDescriptions)
  {
    return lineDescriptions
      .Split("|", StringSplitOptions.TrimEntries)
      .Select(ParseLineDescription)
      .ToArray();
  }

  private static int[] ParseLineDescription(string lineDescription)
  {
    var indices = new List<int>();

    foreach (var digit in lineDescription)
    {
      var gotIndex = int.TryParse(digit.ToString(), out int index);
      indices.Add(index);
    }
  ...
  public static readonly Piece[] ThePieces =
    new[] {
      ParsePieceDescription(nameof(F), F, F_LINES, F_POLYLINES),
      ParsePieceDescription(nameof(H), H, H_LINES, H_POLYLINES),
      ParsePieceDescription(nameof(I), I, I_LINES, I_POLYLINES),
      ParsePieceDescription(nameof(J), J, J_LINES, J_POLYLINES),
      ParsePieceDescription(nameof(L), L, L_LINES, L_POLYLINES),
      ParsePieceDescription(nameof(N), N, N_LINES, N_POLYLINES),
      ParsePieceDescription(nameof(O), O, O_LINES, O_POLYLINES),
      ParsePieceDescription(nameof(P), P, P_LINES, P_POLYLINES),
      ParsePieceDescription(nameof(R), R, R_LINES, R_POLYLINES),
      ParsePieceDescription(nameof(T), T, T_LINES, T_POLYLINES),
      ParsePieceDescription(nameof(U), U, U_LINES, U_POLYLINES),
      ParsePieceDescription(nameof(V), V, V_LINES, V_POLYLINES),
      ParsePieceDescription(nameof(W), W, W_LINES, W_POLYLINES),
      ParsePieceDescription(nameof(X), X, X_LINES, X_POLYLINES),
      ParsePieceDescription(nameof(Y), Y, Y_LINES, Y_POLYLINES),
      ParsePieceDescription(nameof(Z), Z, Z_LINES, Z_POLYLINES)
  };
}
